 ...
 */
 ...
 class SubscriptionManager {
 private:
     /// Maps symbols to the set of sessions subscribed to them
     std::unordered_map<std::string, std::set<std::shared_ptr<WebSocketSession>>> symbol_subscribers;
     DeriClient& deri_client_;
     quill::Logger* logger;

     
     /// Mutex to protect concurrent access to the subscription map
     std::mutex subscription_mutex;
 
 public:
     SubscriptionManager(DeriClient& deri_client, quill::Logger* logger) : deri_client_(deri_client), logger(logger) {}
 ...
 private:
     ...
     /// Fixed-size buffer for sending messages (avoids heap allocations)
     std::array<char, 16384> send_buffer_;
 ...
 private:
     /**
      * @brief Actual implementation of the send operation
      * @param message The message to send
      */
